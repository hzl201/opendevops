...
| 3D_XX（XX为煤矿简称） | 三维监测数据库 | 同步安全监测、人员定位数据到三维，可在三维调取并展示。 | 矿端一张图服务器 |
| LKJT_HydrologicalMonitor_XX（XX为煤矿简称） | 水文监测节点库 | 存储矿端水文实时和报警数据 | 矿端一张图服务器 |
| LKJT_MinePressure_XX（XX为煤矿简称） | 应力与微震监测节点库 | 存储矿端应力和微震的实时和报警数据 | 矿端一张图服务器 |
| LKJT_PersonLocation_Note_XX（XX为煤矿简称） | 人员定位监测节点库 | 存储矿端人员定位实时和报警数据 | 矿端一张图服务器 |
| LKJT_SafeyMonitor_Node_XX（XX为煤矿简称） | 安全监测监测节点库 | 存储矿端安全监测实时和报警数据 | 矿端一张图服务器 |
...
> 移动端是基于H5最新技术进行打造的，安全性较高。但由于移动客户端手机机能限制，无法做到像PC端一样的数据录入，只能进行检查的查询和录入操作。
